...
function lookOrderDetail(index){
	...
	result += '<tr><td width="118">订单号：</td><td>' + order.orderNo +  '</td></tr>';
			...
			if(orderType ==1 || orderType ==3){
				result += '<tr><td>账户名：</td><td>' + trimNull(order.acctName) +  '</td></tr>';
			...
			result  +='</table>';
	layer.open({
		title:order.merchNo + " " + order.orderNo  + " 订单详情",
		area : [ '500px', '600px' ],
		content: result
	});
}
			...
			success : function(data) {
				if (data.code == 1) {
					if(data.data){
						...
						for ( var i in datas) {
							$selMerch.append("<option value='"+datas[i].merchNo+"'>"+datas[i].merchantsName +"</option>");
						}
					}
				} else {
			...
			success : function(data) {
				if (data.code == 1) {
					if(data.data){
						...
						for ( var i in datas) {
							$selAgent.append("<option value='"+datas[i].agentNumber+"'>"+datas[i].agentName+"</option>");
						}
					}
				} else {
					parent.layer.alert(data.msg);
