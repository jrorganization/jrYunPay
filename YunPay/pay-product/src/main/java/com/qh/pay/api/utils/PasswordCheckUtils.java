...
public class PasswordCheckUtils {
    ...
    public static R check(String fundPassword,String username){
        //校验资金密码
        String salt = CacheUtils.getLoginSalt(username);
        ...
        UserDO user = userDaoSt.getByUserName(username);
        if (user == null || !fundPassword.equals(user.getFundPassword())) {
            return R.error("资金密码不正确!");
        }
        return new R();
    }
