...
public class PayQrConfigController {
	...
	R batchUploadQrs(@PathVariable("outChannel") String outChannel, @PathVariable("merchNo") String merchNo,
		...
		if(R.ifSucc(r)){
			try {
				FileUtil.uploadFile(file.getBytes(), RedisUtil.getSysConfigValue(CfgKeyConst.qr_money_path) + merchNo + File.separator + outChannel + File.separator + accountNo + File.separator,
