...
public class YinShengBaoService {
	...
	private static final String url="https://www.unspay.com/delegate-pay-front/delegatePay/fourElementsPay";
...
//			map.put("merchantKey", key);
	...
	private R order_acp(Order order){
		...
		try {
			...
			Map<String,String> map = new HashMap<String,String>();
			map.put("accountId","2120181114093727001");
			...
			map.put("idCardNo","341226197810206533");
			map.put("summary","dsd");
			map.put("phoneNo","18872451160");
			//String responseUrl = PayService.commonAcpNotifyUrl(order);
			//String responseUrl = "http://49.4.91.134:8182/pay/notify/ysb/"+order.getMerchNo()+"/"+order.getOrderNo();
			String responseUrl = "http://119.3.39.3:8182/pay/notify/ysb/"+order.getMerchNo()+"/"+order.getOrderNo();
			map.put("responseUrl", responseUrl);
			//map.put("businessType", "100001");
			map.put("version", "1.0.0");
			//String key = RedisUtil.getPayCommonValue(payMerch + YinShengBaoConst.KEY);
			String key ="jurong123";
			/**参与验签的字段*/
			String sign = "accountId="+map.get("accountId")+"&name="+map.get("name")+"&cardNo="+map.get("cardNo")+"&orderId="+orderId+"&purpose="+map.get("purpose") +"&amount="+map.get("amount")
						+"&idCardNo="+map.get("idCardNo")+"&summary="+map.get("summary")+"&phoneNo="+map.get("phoneNo")+"&responseUrl="+map.get("responseUrl")
						+"&version="+map.get("version") +"&key="+key;
			...
			order.setOrderState(OrderState.ing.id());
			order.setClearState(ClearState.init.id());
