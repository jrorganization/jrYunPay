...
public interface MerchantDao {
    @Select("select merch_no,merchants_name,IFNULL(contacts,'') contacts from merchant where parent_agent = #{parentAgent}")
    @Results({
        @Result(column = "merch_no", jdbcType = JdbcType.VARCHAR, property = "merchNo", typeHandler = StringTypeHandler.class),
        @Result(column = "merchants_name", jdbcType = JdbcType.VARCHAR, property = "merchantsName", typeHandler = StringTypeHandler.class),
