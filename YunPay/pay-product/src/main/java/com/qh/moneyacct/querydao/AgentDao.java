...
public interface AgentDao {
    ...
    @Select("select agent_number,agent_name,IFNULL(contacts,'') contacts from agent where level = 1")
    @Results({
        @Result(column = "agent_number", jdbcType = JdbcType.VARCHAR, property = "agentNumber", typeHandler = StringTypeHandler.class),
        @Result(column = "agent_name", jdbcType = JdbcType.VARCHAR, property = "agentName", typeHandler = StringTypeHandler.class),
    ...
    @Select("select agent_number,agent_name,IFNULL(contacts,'') contacts from agent where parent_agent = #{parentAgent}")
    @Results({
        @Result(column = "agent_number", jdbcType = JdbcType.VARCHAR, property = "agentNumber", typeHandler = StringTypeHandler.class),
        @Result(column = "agent_name", jdbcType = JdbcType.VARCHAR, property = "agentName", typeHandler = StringTypeHandler.class),
        @Result(column = "contacts", jdbcType = JdbcType.VARCHAR, property = "display", typeHandler = StringTypeHandler.class), 
